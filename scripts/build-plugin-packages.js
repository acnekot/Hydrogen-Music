...
function writeTextFile(outputDir, filePath, content) {
...
function buildPluginPackage(plugin) {
  ...
  if (Array.isArray(plugin.runtimeComponents)) {
    for (const component of plugin.runtimeComponents) {
      const componentSourcePath = path.join(rootDir, component.from);
      const componentSource = fs.readFileSync(componentSourcePath, 'utf-8');
      const compiledComponent = compileVueComponent({
        source: componentSource,
        filename: path.basename(component.from),
        id: component.id
      });
      const componentOutputPath = path.join(outputDir, component.to);
      ensureDir(path.dirname(componentOutputPath));
      fs.writeFileSync(componentOutputPath, compiledComponent, 'utf-8');
    }
  }
  ...
  if (Array.isArray(plugin.generatedFiles)) {
    for (const generated of plugin.generatedFiles) {
      writeTextFile(outputDir, generated.to, generated.content);
    }
  }
}
...
const plugins = [
  ...
  },
  {
    ...
    viewReplacements: [
      ["../modules/lyricVisualizerPlugin", "./index.js"],
      ["../../store/playerStore", "./runtime/playerStore.js"],
      ["../../components/Selector.vue", "./runtime/Selector.js"]
    ],
    dependencies: [
      {
        name: 'vue',
    ...
    extraFiles: [
      ...
      { from: 'src/utils/domHandler.js', to: 'utils/domHandler.js' }
    ],
    generatedFiles: [
      {
        to: 'store/playerStore.js',
        content: "export * from '../runtime/playerStore.js';\nexport { usePlayerStore } from '../runtime/playerStore.js';"
      }
    ],
    runtimeComponents: [
      { from: 'src/components/Selector.vue', to: 'runtime/Selector.js', id: 'plugin-selector' }
    ],
    manifest: {
      name: 'lyric-visualizer',
      displayName: '歌词可视化',
