...
const ensurePluginDirectoryBridge = () => {
    if (typeof window === 'undefined') return null
    const bridge = window?.electronAPI
    if (!bridge || typeof bridge.getPluginPath !== 'function' || typeof bridge.setPluginPath !== 'function') {
        console.warn('[PluginStore] 插件目录 API 未注入，路径管理不可用')
        return null
    }
    return bridge
}
...
export const usePluginStore = defineStore('pluginStore', {
    ...
    actions: {
        ...
        async loadDirectoryInfo() {
            const bridge = ensurePluginDirectoryBridge()
            const api = ensurePluginApi()
            if (!bridge && !api) return
            try {
                const info = bridge ? await bridge.getPluginPath() : await api.getDirectory()
                if (info) {
        ...
        async changeDirectory(requestFn) {
            const pluginApi = ensurePluginApi()
            const bridge = ensurePluginDirectoryBridge()
            if (!pluginApi || !bridge || typeof requestFn !== 'function') return null
            ...
            try {
                const result = await requestFn({ pluginApi, bridge })
        ...
        async chooseDirectory() {
            return this.changeDirectory(async ({ pluginApi, bridge }) => {
                const selection = await pluginApi.selectDirectory()
                if (!selection) {
                    return null
                }
                return bridge.setPluginPath(selection)
            })
        },
        async useDefaultDirectory() {
            return this.changeDirectory(({ bridge }) => bridge.setPluginPath(null))
        },
        async setDirectoryManually(directoryPath) {
            ...
            return this.changeDirectory(({ bridge }) => bridge.setPluginPath(target))
        },
    },
})
