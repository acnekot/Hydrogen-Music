<script setup>
import { ref, watch, onMounted, onUnmounted, nextTick, computed, reactive, provide } from 'vue';
...
import { LyricPluginContextSymbol, useLyricOverlayComponents } from '../plugins';
...
const lyricPluginContext = reactive({
    lyricScroll,
    lyricScrollArea,
    lyricAreaVisible,
    lyricPlaceholderVisible,
});

provide(LyricPluginContextSymbol, lyricPluginContext);

const lyricOverlays = useLyricOverlayComponents('before-lyrics');
...
<template>
    <div class="lyric-container" :class="lyricContainerClasses" :style="lyricBackgroundStyle">
        <div v-if="lyricOverlays.length" class="lyric-plugin-overlays">
            <component
                v-for="overlay in lyricOverlays"
                :key="overlay.id"
