...
const VISUALIZER_MAX_FPS = 60;
const VISUALIZER_FRAME_INTERVAL = 1000 / VISUALIZER_MAX_FPS;
const VISUALIZER_MAX_FRAME_STEPS = 3;
...
const startVisualizerLoop = ({ force = false } = {}) => {
    ...
    visualizerFrameAccumulator = VISUALIZER_FRAME_INTERVAL;
    const step = () => {
        if (!shouldShowVisualizer.value || !canvasCtx || !lyricVisualizerCanvas.value) {
            animationFrameId = null;
            visualizerLastFrameTime = 0;
        ...
        const now = getVisualizerTimestamp();
        if (!visualizerLastFrameTime) {
            visualizerLastFrameTime = now;
        }
        const delta = Math.min(
            now - visualizerLastFrameTime,
            VISUALIZER_FRAME_INTERVAL * VISUALIZER_MAX_FRAME_STEPS
        );
        visualizerLastFrameTime = now;
        visualizerFrameAccumulator = Math.min(
        ...
        let keepGoing = true;
        let steps = 0;
        while (visualizerFrameAccumulator >= VISUALIZER_FRAME_INTERVAL && steps < VISUALIZER_MAX_FRAME_STEPS) {
            keepGoing = renderVisualizerFrame();
            steps += 1;
            visualizerFrameAccumulator -= VISUALIZER_FRAME_INTERVAL;
