...
import { usePluginSlot } from '../plugins/manager';
...
const lyricPluginEntries = usePluginSlot('lyric:before-content', () => ({
    refs: {
        lyricScroll,
    },
}));
let lyricScrollWheelHandler = null;
let lyricScrollElement = null;
// 是否存在歌词列表与有效原文内容
...
onMounted(() => {
    ...
    if (scrollEl) {
        ...
        lyricScrollWheelHandler = handler;
        lyricScrollElement = scrollEl;
    }
...
onUnmounted(() => {
    ...
    if (lyricScrollElement && lyricScrollWheelHandler) {
        lyricScrollElement.removeEventListener('wheel', lyricScrollWheelHandler);
    }
    lyricScrollWheelHandler = null;
    lyricScrollElement = null;
});
</script>
...
<template>
    <div class="lyric-container" :class="lyricContainerClasses" :style="lyricBackgroundStyle">
        <component
            v-for="entry in lyricPluginEntries"
            :key="entry.key"
            :is="entry.component"
            v-bind="entry.props"
        />
