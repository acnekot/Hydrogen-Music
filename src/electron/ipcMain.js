...
module.exports = IpcMainEvent = (win, app, lyricFunctions = {}) => {
    ...
    const getDefaultPluginsRoot = () => path.join(app.getAppPath(), 'plugins')
    ...
    const legacyDefaultRoot = normalizeDirectory(path.join(app.getPath('userData'), 'plugins', 'installed'))
    const storedRoot = normalizeDirectory(pluginStorageStore.get('rootDirectory'))
    const defaultRoot = normalizeDirectory(getDefaultPluginsRoot())
    let pluginsRoot
    if (!storedRoot || storedRoot === legacyDefaultRoot) {
        pluginsRoot = defaultRoot
    } else {
        pluginsRoot = storedRoot
    }
    let managedPluginDirectories = new Set(
        (pluginStorageStore.get('directories') || []).map((dir) => normalizeDirectory(dir))
    )
    if (pluginsRoot === defaultRoot) {
        managedPluginDirectories = new Set(
            Array.from(managedPluginDirectories).filter((dir) => {
                if (!dir) return false
                if (dir === pluginsRoot) return true
                return dir.startsWith(`${pluginsRoot}${path.sep}`)
            })
        )
    }
