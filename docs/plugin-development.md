...
> 提示：插件包内可以包含任意层级的文件与子目录，包括 `assets/`、`locales/`，甚至 `node_modules/`。打包时只需保证路径中不出现 `..` 即可，插件管理器会自动映射相对依赖，并支持读取压缩包中嵌套的文件。若插件依赖第三方库，可将精简后的 ES Module 版本放入 `node_modules/<pkg>` 目录（至少包含 `package.json` 与入口文件），运行时即可像平时一样 `import 'pkg'`。

仓库根目录下的 `plugins/` 文件夹现在直接保存了默认歌词可视化与音效增强插件解包后的文件夹结构（包含 `manifest.json`、`index.js` 以及编译后的设置视图脚本）。执行 `node scripts/build-plugin-packages.js` 会重新生成这些文本文件及其依赖：

- 自动将 Vue、Pinia、Howler 等运行时所需的依赖复制到插件内的 `node_modules/`，确保 `.hym` 导入时能直接解析裸模块引用。
- 同步复制示例插件的运行时代码（例如歌词插件使用的 `playerStore`），方便直接打包或二次开发。
- 额外生成兼容层文件（如 `store/playerStore.js`），即便旧版本代码仍引用应用内的路径也能在解包后的插件中正确解析。

如需在其他环境中分发，可在对应文件夹中执行 `zip -r <name>.hym .` 手动生成压缩包，再通过设置页导入。
