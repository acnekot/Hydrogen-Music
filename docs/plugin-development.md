...
`src/plugins/modules/mixerConsolePlugin.js` 演示了一个更完整的插件：它会在应用路由中注册 `/plugins/mixer`，并通过 `settings.route`
为设置页提供“打开混音台”的按钮。页面组件位于 `src/plugins/views/MixerConsole.vue`，包含 6 个常见声道以及主控通道的音量、声像和
均衡控制示例，可作为构建音频处理或视觉化工具的基础模板。
