...
const createLyricWindow = () => {
    ...
    const lyricHtml = path.join(process.env.DIST, 'dist/src/plugins/builtin/desktopLyric/window.html')
    if (process.resourcesPath.indexOf(path.join('node_modules')) != -1) {
        lyricWin.loadURL('http://localhost:5173/src/plugins/builtin/desktopLyric/window.html')
    } else {
